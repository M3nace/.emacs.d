...
- {{{kbd(Alt-0)}}} supprime la frame active
- {{{kbd(Alt-1)}}} supprime la frame inactive
- {{{kbd(Alt-2)}}} split la frame horizontalement
- {{{kbd(Alt-3)}}} split la frame verticalement
- {{{kbd(Alt-<flèches directionnelles>)}}} navigue entre les frames
...
En tapant ~button.btn.btn-default~, un coup de {{{kbd(C-j)}}} et pouf (non ça fait pas des chocapics),
